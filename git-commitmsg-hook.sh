#!/bin/sh
# Agent Diff Gate commit-msg hook — "LOG BEFORE FIXING": a code commit cannot
...
#     git commit -m "fix r3 scope (AREA: R3 try-scope reset misses typed except)"
# The hook extracts the AREA and runs:  check_diff.py --has-entry "<AREA>"
#
...
#                          match check_diff.py's LOG_FILE constant)
#   PYTHON               — python interpreter to use (default: python)
#   AGENT_DIFF_GATE_DIR  — folder that contains check_diff.py, when it is
...
CHECKER="${AGENT_DIFF_GATE_DIR:-$ROOT}/check_diff.py"
PY="${PYTHON:-python}"

if [ ! -f "$CHECKER" ]; then
    echo "commit-msg: cannot find $CHECKER"
    echo "  Place the agent-diff-gate folder at the repo root, or set"
    echo "  AGENT_DIFF_GATE_DIR to the folder that contains check_diff.py."
    exit 1
fi
...
if printf '%s\n' "$staged" | grep -qx "$LOGPATH"; then
    "$PY" "$CHECKER" --logfile "$ROOT/$LOGPATH" || {
...
if [ -z "$area" ]; then
    ...
    echo "  Log the error first:  python check_diff.py --add --area \"<what broke>\" ..."
    echo "  Then commit with:     git commit -m \"... (AREA: <what broke>)\""
    exit 1
fi

"$PY" "$CHECKER" --logfile "$ROOT/$LOGPATH" --has-entry "$area"
if [ $? -ne 0 ]; then
    echo "commit-msg BLOCKED: the error \"$area\" is NOT logged in $LOGPATH."
    echo "  LOG BEFORE FIXING: add an entry first, then commit again."
    exit 1
fi
